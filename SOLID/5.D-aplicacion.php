<?php

#intermediario (abstracion)
interface UsuarioInterface{
    public function verPerfil();
}

#definir las clases concretas (tipos Usuarios)
class Estudiante implements UsuarioInterface{
    public $nombre;
    public $carnet;

    public function __construct($nombre, $carnet)
    {
        $this->nombre = $nombre;
        $this->carnet = $carnet;
    }

    public function verPerfil()
    {
        echo "Soy estudiante";
    }
}

class Maestro implements UsuarioInterface{
    public $nombre;
    public $dui;

    public function __construct($nombre, $dui)
    {
        $this->nombre = $nombre;
        $this->dui = $dui;
    }

    public function verPerfil()
    {
        echo "Soy maestro";
    }
}

class Director implements UsuarioInterface{
    public $nombre;
    public $dui;

    public function __construct($nombre, $dui)
    {
        $this->nombre = $nombre;
        $this->dui = $dui;
    }

    public function verPerfil()
    {
        echo "Soy director";
    }
}

class GestorInscripciones {
    #el atributo debe de recibir un objeto de tipo UsuarioInterface
    # (Estudiante / Maestro)
    private UsuarioInterface $usuario;

    #incializar un objeto (UsuarioInterface) -> (Estudiante / Maestro)
    public function __construct(UsuarioInterface $usuario) { //todas las clases que implementen la interfaz las puedo utilizar AQUI
        $this->usuario = $usuario;
    }
}

$juanito = new Estudiante("juanito perez","J20025");

$marvin = new Maestro("Marvin Reyes","05678909-9");
$rocio = new Director("Rocio","678909-0");

$nueva_inscripcion = new GestorInscripciones($juanito);
$nueva_inscripcion2 = new GestorInscripciones($marvin);
$nueva_inscripcion3 = new GestorInscripciones($rocio);